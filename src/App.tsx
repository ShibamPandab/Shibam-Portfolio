...
export default function App() {
  // loading = true  → Preloader is mounted and covering everything
  // loading = false → Preloader is unmounted, portfolio is fully interactive
  const [loading, setLoading] = useState(true);
  const [activeSection, setActiveSection] = useState('home');

  // Section tracking — starts once the DOM sections exist (they always do now)
  useEffect(() => {
  ...
  }, []); // Runs once — portfolio sections are always in the DOM

  return (
    <>
      {/*
        Cinematic preloader overlay.
        Conditionally rendered — React unmounts it once onComplete() fires.
        The portfolio below is always rendered so it exists under the
        slide-up exit animation before the Preloader unmounts.
      */}
      {loading && <Preloader onComplete={() => setLoading(false)} />}

      {/*
        Portfolio — always rendered, sits behind the preloader (z-index < 9999).
        Becomes interactive the moment the preloader slides away.
      */}
      <div className="relative min-h-screen bg-[#050505] text-[#FFFFFF] font-sans antialiased selection:bg-[#D4AF37] selection:text-[#050505] overflow-x-hidden">
        ...
        {/* Custom cursor (hidden under preloader naturally) */}
        <CustomCursor />
